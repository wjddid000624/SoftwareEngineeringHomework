...
public class Bank {
	...
	List<Account> findAccountsInInterval(int lower, int upper) {
		...
		for(int account: accs.keySet() ){
			if(account >= lower && account <= upper) {
				accounts.add( accs.get(account) );
			}
		}
		return accounts;
	}
	...
	Account findAccountByName(String name) throws NotFoundException {
		// TODO implement this
		Account findAccount = accsByName.get(name);
		if(findAccount == null) {
	...
	Account createAccount(String name, ACCTYPE accType, double initial) {
		...
		if(accType == ACCTYPE.HIGH){
			account = new HighInterest(name, accNumCounter, initial);
		}
		else{
	...
	Account maxBalance() throws EmptyException {
		...
		for(Account account: accs.values()) {
			...
			if(thisBalance > maxBalance) {
				// bigger than maximum balance
				maxBalance = thisBalance;
				maxAccount = account;
			}
			else if(thisBalance == maxBalance && maxAccount.getAccountNumber() > account.getAccountNumber()) {
				// same balance but smaller account number
				maxAccount = account;
			}
		}
		return maxAccount;
